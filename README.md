...
An R-based tool designed to calculate Vegetation Integrity (VI) scores for plant communities using observations exported from iNaturalist, aligned with the NSW Biodiversity Assessment Method (BAM). 📄 Overview
...
To run the scripts, you will need R and RStudio installed, along with the tidyverse package: R

install.packages("tidyverse")

Data Inputs

The script relies on three main CSV files:
...
Open the R project / script in RStudio.

Update the file paths in the master script to point to your data source files (local or remote raw GitHub links).

Run the master script to process the data and generate VI score outputs.
...
Field observation data processed by this tool originates from user submissions on iNaturalist.
